...
void CMainView::showMenu() {
	...
	std::cout << "Author：wale" << std::endl;
	std::cout << "2020/09/15" << std::endl;
...
void  CMainView::findStuAtView(const std::map<int, CStudent> & m1) {
	...
	CStudentMg cStuMg;
	CStudent cStu;
	cStu = cStuMg.locatedStudentbyid(m1, id);
...
void   CMainView::deleteByIdAtView(std::map<int, CStudent> & v1) {
	...
	CStudentMg cStuMg;
	bool b = cStuMg.deleteStudent(v1, id);
...
void   CMainView::updateByIdAtView(std::map<int, CStudent> & m1) {
	...
	CStudent cStu2 = cStuMg.updateStudent(m1, cStu);
